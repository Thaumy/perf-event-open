...
pub(crate) fn from(event_cfg: EventConfig, opts: &Opts) -> Result<Attr> {
    ...
    // The internal variant `__PERF_SAMPLE_CALLCHAIN_EARLY` (Linux 4.18-6.0)
    // widens other enum variants from u32 to u64, so we need to convert
    // the variant to u64 before assigning it to `attr.sample_type`.
    //
    // We leverage the type system here to infer the type of `sample_type` and
    // then assign it to `attr.sample_type` to avoid Clippy warnings about
    // unnecessary casts that depend on the kernel version.
    //
    ...
    // AUX wake-ups use the same epoll path as normal wake-ups, see:

...
pub struct Opts {
    ...
    ///
    /// This means that when this counter group is on a CPU, it should be the only group
    /// using the CPU counters.
    ///
    ...
    /// Enables synchronous signal delivery of `SIGTRAP` to the target process
    /// on event overflow.
    ///
    ...
    /// ```
    ///
    /// NOTE: Child process monitoring itself seems buggy. Panic in the signal handler
    /// may get the child process stuck.
    ///
    ...
    ///
    /// This can make it easier to correlate perf sample times with timestamps generated
    /// by other tools.
    ///
...
pub enum Inherit {
    ...
    ///
    /// This indicates if the process we monitoring creates new tasks (child process or
    /// thread), the counter will count events that happens in these tasks.
    ///
...
/// Controls when to generate a [sample record][crate::sample::record::sample::Sample].
///
/// Defaults to `Count(0)` (no sample mode in `perf record` command), set it to
/// the desired rate to generate sample records.
///
/// The maximum sample rate is specified in
/// `/proc/sys/kernel/perf_event_max_sample_rate`,
/// [`Throttle`][crate::sample::record::throttle::Throttle] record will be generated
/// if the limit has been reached.
///
/// Meanwhile, `/proc/sys/kernel/perf_cpu_time_max_percent` limits the CPU time
/// allowed to handle sampling (0 means unlimited).
/// Sampling will also be throttled if this limit has been reached.
///
/// # Event overflow
///
/// The kernel maintains an unsigned counter with a negative initial value.
/// Every event increments this counter by one, so it will eventually
/// overflow. When overflow occurs, sampling is triggered and the counter
/// is reset. This is what this option controls.
///
/// In addition to asynchronous iterators with the [wake up][WakeUp] option,
/// overflow can also be captured by enabling I/O signaling from
/// the perf event fd, which indicates `POLL_IN` on each overflow.
///
/// Here is an example:
///
/// ```rust
...
pub enum SampleOn {
    ...
    ///
    /// `Count(0)` is the default value for `SampleOn`, it has the same meaning
    /// as `Freq(0)`.
    Count(u64),
}
...
/// Controls the amount of sample skid.
///
/// Skid is how many instructions execute between an event of interest happening
...
/// correspond to which instructions, but hardware is often limited with how
/// small this can be.
///
...
pub struct Lbr {
    ...
    /// Specifies which branches of target privileges can be included in
    /// the branch record.
    ///
    /// If the user does not set privilege level explicitly,
    /// the kernel will inherit the event's privilege level
...
pub struct CallChain {
    ...
    ///
    /// This can move the user stack unwind to a faultable context. It becomes
...
/// When to wake up asynchronous iterators.
///
/// This threshold is a hint from the user. The kernel reserves the right to
/// perform wake-ups independently based on its own scheduling policies.
///
/// "wake up" means notifying the async runtime to schedule the asynchronous
/// iterator's future to be pulled in the next round.
///
/// For performance reasons, we may not want to wake up asynchronous iterators
/// as soon as data is available. With this option we can configure the number
/// of bytes or samples that triggers the wake up.
///
...
pub enum WakeUpOn {
    ...
    ///
    /// The kernel may still trigger a wake-up earlier, typically when the ring buffer
    /// usage reaches half its size in bytes.
    Samples(u32),
}
...
///
/// This data will be copied to the user's signal handler (through `si_perf`
...
pub enum Clock {
    ...
    ///
    /// This clock does not experience discontinuities and backwards jumps
    /// caused by NTP inserting leap seconds as [`RealTime`][Self::RealTime] does.
    ///
    ...
    ///
    /// This clock is affected by discontinuous jumps in the system time (e.g.,
    /// if the system administrator manually changes the clock), and by the incremental
    /// adjustments performed by [`adjtime`](https://www.man7.org/linux/man-pages/man3/adjtime.3.html)
    /// and NTP.
    RealTime,
    ...
    ///
    /// This clock is not affected by discontinuous jumps in the system time (e.g.,
    /// if the system administrator manually changes the clock), but is affected
    /// by the incremental adjustments performed by
    /// [`adjtime`](https://www.man7.org/linux/man-pages/man3/adjtime.3.html) and NTP.
    ///
    /// This time never goes backwards, but successive calls may return identical

...
const PATCHES_DIR: &str = "bindgen/patches";
...
fn main() -> Result<()> {
    ...
    if let Err(e) = generate_bindings(&bindings_dir) {
        fs::remove_dir_all(&bindings_dir)
            .with_context(|| format!("failed to generate bindings: {:?}", e))
            .with_context(|| format!("failed to remove dir: {:?}", bindings_dir))?;

        bail!("failed to generate bindings: {:?}", e);
    }

    Ok(())
}

fn generate_bindings(to: &Path) -> Result<()> {
    let mut patch_paths = fs::read_dir(PATCHES_DIR)
        ...
        .map(|it| {
            ...
            // linux-6.13.rs.patch -> 6.13.rs.patch
    ...
    for (_, _, patch_path) in patch_paths {
        ...
        let binding_file_name = patch_path
            .file_stem()
            .context("failed to generate binding file name")?;
        let binding_path = to.join(binding_file_name);
